...
# This version does an exhaustive search through all possible orderings until it finds one
import re
import sys
# We'll use "shuffle" to randomly change the rule order
from random import shuffle
# We'll use a permutation function for exhaustive search, from itertools
import itertools
...
# Changing the grammar here consists of stepping through all possible orders. We can get those up from, with "permutate"
searchorder = itertools.permutations(rules)

current_order = 0

while number_correct < len(inputs):
	# Try the next order of rules
	current_order += 1
	rules = list(next(searchorder))
