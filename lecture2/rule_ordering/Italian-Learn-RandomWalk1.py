...
# This version does a random walk, changing just one relative ordering of two adjacent rules at a time.
import re
import sys
# We'll use "shuffle" and "randinit" to randomly change the rule order
import random
...
while number_correct < len(inputs):
	...
	# In this script, we swap the order of a pair of adjacent rules. 
	# Pick a random rule to swap with the one that comes after it
	i = random.randint(0,len(rules)-2)
	# Now swap them
